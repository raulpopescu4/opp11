...
void GUI::initGUI(){
    ...
    adminLayout->addWidget(this->undoButton);
    adminLayout->addWidget(this->redoButton);
    ...
    this->watchlistTableModel = new WatchlistTable(this->watchlist);

    this->watchlistTableView = new QTableView();
    this->watchlistTableView->setModel(this->watchlistTableModel);
    userPageLayout->addWidget(this->watchlistTableView);
...
void GUI::connectSignalAndSlots() {
    ...
    QObject::connect(this->undoButton, SIGNAL(clicked()), this, SLOT(undoButtonSlot()));

    QObject::connect(this->redoButton, SIGNAL(clicked()), this, SLOT(redoButtonSlot()));
}

void GUI::undoButtonSlot(){

    if(this->commandManager.undoSize() == 0){
        QMessageBox error;
        error.critical(0, "Error", "No undo can be done.");
    }
    else{
...
void GUI::redoButtonSlot(){

    if(this->commandManager.redoSize() == 0){
        QMessageBox error;
        error.critical(0, "Error", "No redo can be done.");
    }
    else{
...
void GUI::addTutorial(const std::string &title, const std::string &presenter, const std::string &duration,
    ...
    if(ok==1){
        ...
        BaseCommand* newCommand(new AddCommand(&this->tutorials, t));
...
void GUI::removeTutorialHandler() {
    ...
    BaseCommand* newCommand(new RemoveCommand(&this->tutorials, idx));
    commandManager.executeCommand(newCommand);
...
void GUI::addWatchlistHandler() {
    ...
    this->tutorialsListTwo->takeItem(this->tutorialsListTwo->currentRow());

    delete this->watchlistTableModel;
    this->watchlistTableModel = new WatchlistTable(this->watchlist);
    this->watchlistTableView->setModel(this->watchlistTableModel);
...
void GUI::removeWatchlistHandler() {
    ...
    QModelIndexList selected = this->watchlistTableView->selectionModel()->selectedRows();
    QModelIndex index;
    ...
    QString itemInList = QString::fromStdString(this->watchlist[idx].getTitle());
    QFont f("Century Gothic", 12, QFont::Bold);
    QListWidgetItem* item = new QListWidgetItem{itemInList};
    item->setFont(f);
    this->tutorialsListTwo->addItem(item);
...
void GUI::updateTutorialHandler() {
    ...
    std::string title = this->titleEdit->toPlainText().toStdString();
    ...
    std::string link = this->linkEdit->toPlainText().toStdString();
    int ok = 1;
    Validator valid;
    try{
        valid.valid_title(title);
    ...
    if(ok==1){
        ...
        BaseCommand* newCommand(new UpdateCommand(&this->tutorials, idx, t));
        commandManager.executeCommand(newCommand);
